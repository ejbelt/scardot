...
namespace scardot
{
    ...
    public readonly partial struct Callable
    {
        ...
        private readonly unsafe delegate* managed<object, NativeVariantPtrArgs, out scardot_variant, void> _trampoline;
        ...
        /// </summary>
        public unsafe delegate* managed<object, NativeVariantPtrArgs, out scardot_variant, void> Trampoline
            => _trampoline;
        ...
        {
            using scardot_callable callable = Marshaling.ConvertCallableToNative(this);

            int argc = args.Length;

            Span<scardot_variant.movable> argsStoreSpan = argc <= VarArgsSpanThreshold ?
                stackalloc scardot_variant.movable[VarArgsSpanThreshold] :
                new scardot_variant.movable[argc];

            Span<IntPtr> argsSpan = argc <= VarArgsSpanThreshold ?
                stackalloc IntPtr[VarArgsSpanThreshold] :
                new IntPtr[argc];

            fixed (scardot_variant* varargs = &MemoryMarshal.GetReference(argsStoreSpan).DangerousSelfRef)
            fixed (IntPtr* argsPtr = &MemoryMarshal.GetReference(argsSpan))
            {
                for (int i = 0; i < argc; i++)
                {
                    varargs[i] = (scardot_variant)args[i].NativeVar;
                    argsPtr[i] = new IntPtr(&varargs[i]);
                }

                scardot_variant ret = NativeFuncs.scardotsharp_callable_call(callable,
                    (scardot_variant**)argsPtr, argc, out scardot_variant_call_error vcall_error);
                ExceptionUtils.DebugCheckCallError(callable, (scardot_variant**)argsPtr, argc, vcall_error);
        ...
        public unsafe void CallDeferred(params Variant[] args)
        {
            ...
            fixed (IntPtr* argsPtr = &MemoryMarshal.GetReference(argsSpan))
            {
                ...
                NativeFuncs.scardotsharp_callable_call_deferred(callable, (scardot_variant**)argsPtr, argc);
            }
        }
        ...
        ///    <item>
        ///        <term>args</term>
        ///        <description>Array of <see cref="scardot_variant"/> arguments.</description>
        ///    </item>
        ///    <item>
        ///        <term>ret</term>
        ///        <description>Return value of type <see cref="scardot_variant"/>.</description>
        ///    </item>
        ///</list>
        /// <para>
        ...
        ///
        /// <code>
        ///     static void Trampoline(object delegateObj, NativeVariantPtrArgs args, out scardot_variant ret)
        ///     {
        ///         if (args.Count != 1)
