...
namespace scardot.NativeInterop
{
    internal static class ExceptionUtils
    {
        ...
        private static void SendToScriptDebugger(Exception e)
        {
            ...
            using scardot_string nFile = Marshaling.ConvertStringToNative(file);
            using scardot_string nFunc = Marshaling.ConvertStringToNative(func);
            using scardot_string nErrorMsg = Marshaling.ConvertStringToNative(errorMsg);
            using scardot_string nExcMsg = Marshaling.ConvertStringToNative(excMsg.ToString());

            using DebuggingUtils.scardot_stack_info_vector stackInfoVector = default;
            ...
            unsafe
            {
                ...
                {
                    DebuggingUtils.scardot_stack_info* stackInfo = &stackInfoVector.Elements[i];
                ...
                NativeFuncs.scardotsharp_internal_script_debugger_send_error(nFunc, nFile, line,
                    nErrorMsg, nExcMsg, scardot_error_handler_type.ERR_HANDLER_ERROR, stackInfoVector);
            }
        }
        ...
        public static void LogException(Exception e)
        {
            try
            {
                if (NativeFuncs.scardotsharp_internal_script_debugger_is_active().ToBool())
                {
        ...
        [Conditional("DEBUG")]
        public unsafe static void DebugCheckCallError(scardot_string_name method, IntPtr instance, scardot_variant** args, int argCount, scardot_variant_call_error error)
        {
            if (error.Error != scardot_variant_call_error_error.SCARDOT_CALL_ERROR_CALL_OK)
            {
                using scardot_variant instanceVariant = VariantUtils.CreateFromscardotObjectPtr(instance);
        ...
        [Conditional("DEBUG")]
        public unsafe static void DebugCheckCallError(in scardot_callable callable, scardot_variant** args, int argCount, scardot_variant_call_error error)
        {
            if (error.Error != scardot_variant_call_error_error.SCARDOT_CALL_ERROR_CALL_OK)
            {
                using scardot_variant callableVariant = VariantUtils.CreateFromCallableTakingOwnershipOfDisposableValue(callable);
        ...
        private unsafe static string GetCallErrorWhere(ref scardot_variant_call_error error, scardot_string_name method, scardot_variant* instance, scardot_variant** args, int argCount)
        {
            string? methodstr = null;
            string basestr = GetVariantTypeName(instance);

            if (method == scardotObject.MethodName.Call || (basestr == "scardot.TreeItem" && method == TreeItem.MethodName.CallRecursive))
            {
                if (argCount >= 1)
                {
                    methodstr = VariantUtils.ConvertToString(*args[0]);
                    if (error.Error == scardot_variant_call_error_error.SCARDOT_CALL_ERROR_CALL_ERROR_INVALID_ARGUMENT)
                    {
                        error.Argument += 1;
                    }
                }
        ...
        private unsafe static string GetCallErrorMessage(scardot_variant_call_error error, string where, scardot_variant** args)
        {
            switch (error.Error)
            {
                case scardot_variant_call_error_error.SCARDOT_CALL_ERROR_CALL_ERROR_INVALID_ARGUMENT:
                {
                ...
                case scardot_variant_call_error_error.SCARDOT_CALL_ERROR_CALL_ERROR_TOO_MANY_ARGUMENTS:
                case scardot_variant_call_error_error.SCARDOT_CALL_ERROR_CALL_ERROR_TOO_FEW_ARGUMENTS:
                    return $"Invalid call to {where}. Expected {error.Expected} arguments.";
                case scardot_variant_call_error_error.SCARDOT_CALL_ERROR_CALL_ERROR_INVALID_METHOD:
                    return $"Invalid call. Nonexistent {where}.";
                case scardot_variant_call_error_error.SCARDOT_CALL_ERROR_CALL_ERROR_INSTANCE_IS_NULL:
                    return $"Attempt to call {where} on a null instance.";
                case scardot_variant_call_error_error.SCARDOT_CALL_ERROR_CALL_ERROR_METHOD_NOT_CONST:
        ...
        private unsafe static string GetVariantTypeName(scardot_variant* variant)
        {
            if (variant->Type == Variant.Type.Object)
            {
                scardotObject obj = VariantUtils.ConvertToscardotObject(*variant);
                if (obj == null)
                {
                    return "null instance";
                }
                else if (!scardotObject.IsInstanceValid(obj))
                {
