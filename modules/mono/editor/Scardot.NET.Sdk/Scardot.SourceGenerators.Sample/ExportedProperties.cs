...
namespace scardot.SourceGenerators.Sample
{
    ...
    public partial class ExportedProperties : scardotObject
    {
        ...
        [Export]
        private scardot.Collections.Dictionary PropertyscardotDictionary { get; set; } =
            new() { { "foo", 10 }, { Vector2.Up, Colors.Chocolate } };

        [Export]
        private scardot.Collections.Array PropertyscardotArray { get; set; } =
            new() { "foo", 10, Vector2.Up, Colors.Chocolate };

        [Export]
        private scardot.Collections.Dictionary<string, bool> PropertyscardotGenericDictionary { get; set; } =
            new() { { "foo", true }, { "bar", false } };

        [Export]
        private scardot.Collections.Array<int> PropertyscardotGenericArray { get; set; } =
            new() { 0, 1, 2, 3, 4, 5, 6 };
    }
}
