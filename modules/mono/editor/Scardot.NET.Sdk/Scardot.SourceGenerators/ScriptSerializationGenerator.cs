...
namespace scardot.SourceGenerators
{
    ...
    public class ScriptSerializationGenerator : ISourceGenerator
    {
        ...
        )
        {
            ...
            var scardotClassProperties = propertySymbols.Where(property => !(property.IsReadOnly || property.IsWriteOnly || property.SetMethod!.IsInitOnly))
                .WhereIsscardotCompatibleType(typeCache)
                .ToArray();
            var scardotClassFields = fieldSymbols.Where(property => !property.IsReadOnly)
                .WhereIsscardotCompatibleType(typeCache)
                .ToArray();
