...
namespace scardotTools.Ides
{
    public sealed class MessagingServer : IDisposable
    {
        ...
        public MessagingServer(string editorExecutablePath, string projectMetadataDir, ILogger logger)
        {
            ...
            _metaFilePath = Path.Combine(projectMetadataDir, scardotIdeMetadata.DefaultFileName);

            // Make sure the directory exists
            Directory.CreateDirectory(projectMetadataDir);

            // The scardot editor's file system thread can keep the file open for writing, so we are forced to allow write sharing...
            ...
            where TResponse : Response, new()
        {
            using (await _peersSem.UseAsync())
            {
                ...
                {
                    _logger.LogError("Cannot write request. No client connected to the scardot Ide Server.");
                    return;
                }
        ...
        private class ServerMessageHandler : IMessageHandler
        {
            private static void DispatchToMainThread(Action action)
            {
                ...
                scardot.Dispatcher.SynchronizationContext.Post(d, null);
            }
            ...
            private static Task<Response> HandleDebugPlay(DebugPlayRequest request)
            {
                DispatchToMainThread(() =>
                {
                    ...
                    scardotSharpEditor.Instance.SkipBuildBeforePlaying = !(request.BuildBeforePlaying ?? true);
                    ...
                    Environment.SetEnvironmentVariable("SCARDOT_MONO_DEBUGGER_AGENT",
                    ...
                    Environment.SetEnvironmentVariable("SCARDOT_MONO_DEBUGGER_AGENT", "");
                    scardotSharpEditor.Instance.SkipBuildBeforePlaying = false;
                });
            ...
            private static async Task<Response> HandleCodeCompletionRequest(CodeCompletionRequest request)
            {
                ...
                await scardot.Engine.GetMainLoop().ToSignal(scardot.Engine.GetMainLoop(), "process_frame");
