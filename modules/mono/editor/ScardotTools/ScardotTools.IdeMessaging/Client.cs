...
namespace scardotTools.IdeMessaging
{
    ...
    public sealed class Client : IDisposable
    {
        ...
        private scardotIdeMetadata scardotIdeMetadata;
        private readonly FileSystemWatcher fsWatcher;

        public string scardotEditorExecutablePath => scardotIdeMetadata.EditorExecutablePath;
        ...
        public Client(string identity, string scardotProjectDir, IMessageHandler messageHandler, ILogger logger)
        {
            this.identity = identity;
            this.messageHandler = messageHandler;
            this.logger = logger;

            string projectMetadataDir = Path.Combine(scardotProjectDir, ".scardot", "mono", "metadata");
            // FileSystemWatcher requires an existing directory
            if (!Directory.Exists(projectMetadataDir))
            {
                // Check if the non hidden version exists
                string nonHiddenProjectMetadataDir = Path.Combine(scardotProjectDir, "scardot", "mono", "metadata");
        ...
        private async void OnMetaFileChanged(object sender, FileSystemEventArgs e)
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (metadata != null && metadata != scardotIdeMetadata)
                {
                    scardotIdeMetadata = metadata.Value;
        ...
        private async Task ConnectToServer()
        {
            ...
            try
            {
                ...
                await tcpClient.ConnectAsync(IPAddress.Loopback, scardotIdeMetadata.Port);
