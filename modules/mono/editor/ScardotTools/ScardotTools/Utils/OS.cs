...
namespace scardotTools.Utils
{
    ...
    public static class OS
    {
        ...
        public static readonly Dictionary<string, string> DotNetOSPlatformMap = new Dictionary<string, string>
        {
            ...
            // scardot has a single export profile for both, named LinuxBSD.
        ...
        {
            Internal.scardot_icall_Utils_OS_GetPlatformName(out scardot_string dest);
            using (dest)
            {
        ...
        private static string? PathWhichUnix(string name)
        {
            ...
            return searchDirs.Select(dir => Path.Combine(dir, name))
                .FirstOrDefault(path =>
                {
                    using scardot_string pathIn = Marshaling.ConvertStringToNative(path);
                    return File.Exists(path) && Internal.scardot_icall_Utils_OS_UnixFileHasExecutableAccess(pathIn);
                });
        }
