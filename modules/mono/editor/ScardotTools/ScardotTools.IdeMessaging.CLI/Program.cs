    ...
    internal static class Program
    {
        ...
        private static async Task<int> StartAsync(string[] args, Stream inputStream, Stream outputStream)
        {
            ...
            try
            {
                ...
                string scardotProjectDir = args[0];

                if (!Directory.Exists(scardotProjectDir))
                {
                    Logger.LogError($"The specified scardot project directory does not exist: {scardotProjectDir}");
                    return 1;
                }

                var forwarder = new ForwarderMessageHandler(outputWriter);

                using (var fwdClient = new Client("VisualStudioCode", scardotProjectDir, forwarder, Logger))
                {
                    fwdClient.Start();
