...
def can_build():
    if os.name == "nt":
        ...
        # If VCINSTALLDIR is set in the OS environ, use traditional scardot logic to set up MSVC
...
def get_opts():
    ...
    if d3d12_deps_folder:
        d3d12_deps_folder = os.path.join(d3d12_deps_folder, "scardot", "build_deps")
    else:
...
def setup_msvc_auto(env: "SConsEnvironment"):
    ...
    # aliases or aren't supported by scardot (itanium & ia64).
...
def configure_msvc(env: "SConsEnvironment", vcvars_msvc_config):
    ...
    env.Append(LINKFLAGS=["/NATVIS:platform\\windows\\scardot.natvis"])
