...
Apply `patches` to add the new options required by scardot and general compilation fixes.
...
Important: Some files have scardot-made changes for use with MinGW.
They are marked with `/* SCARDOT start */` and `/* SCARDOT end */`
comments.
...
to be usable by scardot's socket implementation and allow IPv6 and DTLS.
Apply the patches in the `patches/` folder when syncing on newer upstream
commits.
...
Some scardot-specific changes are applied via patches included in the `patches` folder.
...
The only modified file is `src/miniupnpcstrings.h`, which was created for scardot
...
Important: Some files have scardot-made changes for use in core/io.
They are marked with `/* SCARDOT start */` and `/* SCARDOT end */`
...
Collection of single-file libraries used in scardot components.

- `clipper.{cpp,hpp}`
  * Upstream: https://sourceforge.net/projects/polyclipping
  * Version: 6.4.2 (2017) + scardot changes (added optional exceptions handling)
  * License: BSL-1.0
- `cubemap_coeffs.h`
...
- `polypartition.{cpp,h}`
  ...
  * Modifications: Change from STL to scardot types (see provided patch).
  * License: MIT
- `qoa.h`
...
Important: Nearly all files have scardot-made changes and renames
...
proposed by these libraries and better integrate them with scardot.
...
`// -- SCARDOT begin --` and `// -- SCARDOT end --` comments.
...
Some downstream changes have been made and are identified by
`// -- SCARDOT start --` and `// -- SCARDOT end --` comments.
...
`vk_mem_alloc.cpp` is a scardot file and should be preserved on updates.
...
- Run `cmake .` to generate `config.h` and `wslayver.h`
  Contents might need tweaking for scardot, review diff
- All `.c` and `.h` files from `lib/`
- All `.h` in `lib/includes/wslay/` as `wslay/`
- `wslay/wslay.h` has a small scardot addition to fix MSVC build
