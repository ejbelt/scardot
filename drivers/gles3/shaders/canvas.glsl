...
vec4 light_shadow_compute(uint light_base, vec4 light_color, vec4 shadow_uv
	...
	vec4 shadow_color = scardot_unpackUnorm4x8(light_array[light_base].shadow_color);
#ifdef LIGHT_CODE_USED
...
void main() {
	...
	if (specular_shininess_used || (using_light && normal_used && bool(read_draw_data_flags & FLAGS_DEFAULT_SPECULAR_MAP_USED))) {
		...
		specular_shininess *= scardot_unpackUnorm4x8(read_draw_data_specular_shininess);
